...
class AnsibleDocsPlugin(mkdocs.plugins.BasePlugin[AnsibleDocsPluginConfig]):
    ...
    def on_files(self, files, config):
        ...
        for fqcn in self.config.collections:
            ...
            nf = mkdocs.structure.files.File(f"{fqcn}/index.md", src_dir=None, dest_dir=config.site_dir, use_directory_urls=False)
            ...
            for plugin_type in collection_metadata["all"]:
                ...
                nf = mkdocs.structure.files.File(f"{fqcn}/{plugin_type}.md", src_dir=None, dest_dir=config.site_dir, use_directory_urls=False)
                nf.generated_by = "ansible-docs"
    ...
    def _get_ansible_doc_metadata(fqcn):
        ...
        result = subprocess.run(["ansible-doc", "--metadata-dump", "--no-fail-on-errors", fqcn], capture_output=True)
